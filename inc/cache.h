...
public:
    ...
    // satvik
    int total_eviction_cnt;
    vector<uint64_t> set_access_cnt;
    vector<uint64_t> set_eviction_cnt;
    vector<uint32_t> warmth[4];
    vector<vector<uint32_t>> remap;
    uint64_t reset_eviction_cnt;
    // satvik
    uint32_t LATENCY;
    BLOCK **block;
    int fill_level;
    ...
    {
        remap.assign(NUM_SET, vector<uint32_t>());
        ...
        if (NAME == "LLC")
        {
            set_access_cnt.assign(NUM_SET, 0);
            set_eviction_cnt.assign(NUM_SET, 0);

            reset_eviction_cnt = 0;
        }
        // satvik
        LATENCY = 0;
