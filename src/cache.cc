...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    // satvik
    // if (cache_type == IS_LLC && warmup_complete[0])
    // set_access_cnt[set]++;
    // satvik
    ...
    { // this is a bypass that does not fill the LLC
      // satvik
      // this if condition is never executed in practice,used the following print statement to check it
      // cout << "sat............." << endl;
      // satvik
...
void CACHE::handle_writeback()
{
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
  {
    ...
    uint32_t orig_set = set;
    int way = check_hit(&WQ.entry[index], set);
    // cout << "writeback" << flush;
    if (way >= 0)
    ...
    { // writeback miss (or RFO miss for L1D)
      ...
      else
      {
        ...
        if ((cache_type == IS_LLC) && (way == LLC_WAY))
        {
          std::cerr << "LLC bypassing for writebacks is not allowed!" << endl;
          assert(0);
        }
#endif
...
void CACHE::handle_read()
{
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      { // read hit
        // satvik
        if (cache_type == IS_LLC && warmup_complete[read_cpu])
          set_access_cnt[orig_set]++;
        // satvik
        if (cache_type == IS_ITLB)
        {
...
void CACHE::seperate_set_warmth()
{
  vector<pair<int, int>> miss_index(LLC_SET);

  for (int i = 0; i < LLC_SET; i++)
  {
    miss_index[i] = {set_eviction_cnt[i], i};
  }
  sort(miss_index.begin(), miss_index.end());

  for (int i = 0; i < LLC_SET; i++)
  {
    if (i < LLC_SET / 4)
    {
      warmth[0].push_back(miss_index[i].second);
    }
    else if (LLC_SET / 4 <= i && i < LLC_SET / 2)
    {
      warmth[1].push_back(miss_index[i].second);
    }
    else if (LLC_SET / 2 <= i && i < 3 * LLC_SET / 4)
    {
      warmth[2].push_back(miss_index[i].second);
    }
    else
    {
      warmth[3].push_back(miss_index[i].second);
    }
  }
}
...
{
  for (int i = 0; i < min((int)warmth[3].size(), (int)warmth[0].size()); i++)
  {
    remap[warmth[3][(int)warmth[3].size() - i - 1]].push_back(warmth[0][i]);
  }
  for (int i = 0; i < min((int)warmth[3].size(), (int)warmth[1].size()); i++)
  {
    remap[warmth[3][(int)warmth[3].size() - i - 1]].push_back(warmth[1][i]);
  }
}
...
void CACHE::operate()
{
  ...
  if (/*remapped == false && */cache_type == IS_LLC && total_eviction_cnt > LLC_SET * LLC_WAY)
  {
    //remapped = true;

    seperate_set_warmth();
    remap_sets();
    total_eviction_cnt = 0;
    for(int i=0; i<LLC_SET; i++){
      //set_access_cnt[i] = 0;
      //set_eviction_cnt[i] = 0;
      remap[i].clear();
      remap[i].push_back(i);
    }
    for(int i=0; i<4; i++){
...
int CACHE::check_hit(PACKET *packet, uint32_t &set)
{
  ...
  if (NUM_SET < set)
  {
    ...
    std::cerr << " address: " << hex << packet->address << " full_addr: " << packet->full_addr << dec;
    std::cerr << " event: " << packet->event_cycle << endl;
    assert(0);
  }
  ...
  for (uint32_t i = 0; i < remap[set].size(); i++)
  {
    int subset = remap[set][i];
    for (uint32_t way = 0; way < NUM_WAY; way++)
    {
      if (block[subset][way].valid && (block[subset][way].tag == packet->address))
      {
        match_way = way;
        set = subset;
        found_set = true;
...
int CACHE::invalidate_entry(uint64_t inval_addr)
{
  ...
  {
    std::cerr << "[" << NAME << "_ERROR] " << __func__ << " invalid set index: " << set << " NUM_SET: " << NUM_SET;
    std::cerr << " inval_addr: " << hex << inval_addr << dec << endl;
    assert(0);
  }
...
int CACHE::add_rq(PACKET *packet)
{
  ...
  if (RQ.entry[index].address != 0)
  {
    ...
    std::cerr << " address: " << hex << RQ.entry[index].address;
    std::cerr << " full_addr: " << RQ.entry[index].full_addr << dec << endl;
    assert(0);
  }
#endif
...
int CACHE::add_wq(PACKET *packet)
{
  ...
  {
    std::cerr << "[" << NAME << "_ERROR] " << __func__ << " is not empty index: " << index;
    std::cerr << " address: " << hex << WQ.entry[index].address;
    std::cerr << " full_addr: " << WQ.entry[index].full_addr << dec << endl;
    assert(0);
  }
...
int CACHE::add_pq(PACKET *packet)
{
  ...
  {
    std::cerr << "[" << NAME << "_ERROR] " << __func__ << " is not empty index: " << index;
    std::cerr << " address: " << hex << PQ.entry[index].address;
    std::cerr << " full_addr: " << PQ.entry[index].full_addr << dec << endl;
    assert(0);
  }
#endif
...
void CACHE::return_data(PACKET *packet)
{
  ...
  if (mshr_index == -1)
  {
    ...
    std::cerr << " full_addr: " << hex << packet->full_addr;
    std::cerr << " address: " << packet->address << dec;
    std::cerr << " event: " << packet->event_cycle << " current: " << current_core_cycle[packet->cpu] << endl;
    assert(0);
  }
